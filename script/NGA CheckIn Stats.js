...
// @match        https://ngabbs.com/read.php?tid=20217469*
...
function time_in_range(t) {
    ...
    let now = new Date()
    let flag = 0
    if (now.getMonth() < get_month(t)) {
        flag = 0
    } else if (now.getMonth() == get_month(t)) {
        flag = 1
    } else {
        flag = 2
    }
...
function validate_post(content) {
    ...
    img_list.forEach((img) => {
        ...
        if (!class_) return
        if (!src_url.includes("attachments")) return
        else {
            min_words_length = 10
        }
    })
...
function get_all_post_in_page(doc) {
    ...
    post_tables.forEach((post_div) => {
        let date = post_div
            .querySelector(".c2")
            .querySelector("span[title='reply time']").textContent

        let uid = post_div.querySelector(".c1").querySelector("a").href

        let idx = post_div
            .querySelector(".c1")
            .querySelector("span")
            .id.replace("posterinfo", "")
...
function processPosts(posts) {
    ...
    for (let k = posts.length - 1; k > 0; k--) {
        info = posts[k]
        if (time_in_range(info[1]) == 0) {
            continue
        }
        if (time_in_range(info[1]) == 1) {
...
function downloadTable() {
    ...
    anotherlink.setAttribute("href", anotherUri)
    anotherlink.setAttribute("download", "NGA_Invalid_Reply.csv")
